...
# def class runid
class run(object):
    def __init__(self, dbfile, sf):
        # parse dbfile
        self.runid, self.name, self.line, self.color = parse_dbfile(dbfile)
        self.sf = sf
...
def add_legend(lg, ax):
    ...
    plt.legend(lline, llabel, loc='upper left', ncol = 3, frameon=False)
    lax.set_axis_off() 
# ========================== stat plot ============================
...
# ============================ file parser =====================================
def parse_dbfile(cfile):
    print 'open file '+cfile
    with open(cfile) as fid:
        crunid = find_key('runid', fid)
        cname  = find_key('name' , fid)
        cline  = find_key('line' , fid)
        ccolor = find_key('color', fid)
    # return value
    return crunid, cname, cline, ccolor
...
# parse db file
    ...
    for irun, runid in enumerate(args.runid):
        # initialise run
        run_lst[irun] = run(runid+'.db', args.sf[0])
...
# need to deal with multivar
    for ivar, cvar in enumerate(args.var):
        ...
        # set title
        if (args.title):
            ax[ivar].set_title(args.title[ivar])
    ...
    plt.savefig(args.o[0]+'.png', format='png', dpi=300)
    plt.show()
